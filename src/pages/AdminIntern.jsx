...
import { 
  Users, CheckCircle, XCircle, UserCheck, UserX, ClipboardList, Upload, Calendar, Award, Eye, Edit, Trash2, 
...
const AdminIntern = () => {
  ...
  const [data, setData] = useState({
    stats: { totalInterns: 0, pendingApprovals: 0, activeInterns: 0, completedInterns: 0, avgProgress: 0 },
    interns: [], tasks: [], resources: [], attendance: [], certificates: [], batches: [], activities: []
  });
  ...
  const [forms, setForms] = useState({
    ...
    resource: { title: '', description: '', type: 'pdf', batch: 'All Batches', external_url: '', file: null },
  ...
  const loadData = async (endpoints) => {
    try {
      setLoading(true);
      const results = await Promise.all(endpoints.map(endpoint => apiCall(endpoint)));
      return results;
    } catch (err) {
  ...
  const loadDashboardData = async () => {
    const [statsData, activitiesData] = await loadData(['/admin/dashboard/stats', '/admin/dashboard/activities']);
    setData(prev => ({ ...prev, stats: statsData || prev.stats, activities: activitiesData || [] }));
  };

  const loadInterns = async () => {
    const params = new URLSearchParams({
      page: filters.currentPage, limit: 10,
      ...(filters.searchTerm && { search: filters.searchTerm }),
      ...(filters.filterStatus !== 'all' && { status: filters.filterStatus })
    });
    const [result] = await loadData([`/admin/interns?${params}`]);
  ...
  const updateInternStatus = async (id, status) => {
    try {
      await apiCall(`/admin/interns/${id}/status`, { method: 'PUT', body: JSON.stringify({ status }) });
  ...
  const handleFormSubmit = async (type, isEdit = false) => {
    try {
      const endpoints = {
        task: isEdit ? `/admin/tasks/${modals.editingTask.id}` : '/admin/tasks',
      ...
      if (type === 'resource') {
        ...
        Object.keys(body).forEach(key => {
          if (body[key] !== null && body[key] !== '') formData.append(key, body[key]);
        });
        body = formData;
        headers = {};
      } else {
      ...
      setModals(prev => ({ ...prev, [`show${type.charAt(0).toUpperCase() + type.slice(1)}Modal`]: false, editingTask: null }));
      setForms(prev => ({ ...prev, [type]: type === 'task' ? { title: '', description: '', assigned_to: '', due_date: '' } : prev[type] }));
  ...
  // Utility functions
  const getStatusColor = (status) => ({ pending: '#ff9800', active: '#4caf50', completed: '#2196f3', rejected: '#f44336', inactive: '#9e9e9e' }[status] || '#9e9e9e');
  const formatDate = (dateString) => new Date(dateString).toLocaleDateString();
  const formatTime = (timeString) => timeString ? new Date(`2000-01-01T${timeString}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
  ...
  const buildActionButton = (onClick, variant, size, icon, text) => (
    <button onClick={onClick} className={`aim-btn aim-btn--${variant} ${size ? `aim-btn--${size}` : ''}`}>
      {icon}{text}
    </button>
  );
    ...
    <div className="aim-modal-overlay">
      <div className="aim-modal">
        <div className="aim-modal-header">
          ...
          <button onClick={onClose} className="aim-modal-close"><X className="aim-icon-sm" /></button>
        </div>
        <div className="aim-modal-body">{children}</div>
        <div className="aim-modal-actions">
          <button onClick={onSubmit} className="aim-btn aim-btn--primary">{submitText}</button>
          <button onClick={onClose} className="aim-btn aim-btn--secondary">Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  const renderDashboard = () => (
    <div className="aim-dashboard">
      <div className="aim-stats-grid">
        {buildStatCard(<Users className="aim-icon" />, data.stats.totalInterns, "Total Interns", "aim-stat-icon--blue")}
        {buildStatCard(<Clock className="aim-icon" />, data.stats.pendingApprovals, "Pending Approvals", "aim-stat-icon--orange")}
        {buildStatCard(<UserCheck className="aim-icon" />, data.stats.activeInterns, "Active Interns", "aim-stat-icon--green")}
        {buildStatCard(<Award className="aim-icon" />, data.stats.completedInterns, "Completed", "aim-stat-icon--purple")}
        {buildStatCard(<TrendingUp className="aim-icon" />, `${data.stats.avgProgress}%`, "Avg Progress", "aim-stat-icon--indigo")}
      </div>
      <div className="aim-card">
        <div className="aim-card-header"><h3 className="aim-card-title">Recent Activities</h3></div>
          ...
          <div className="aim-activities">
            {data.activities.slice(0, 5).map((activity, index) => (
              <div key={index} className="aim-activity">
                <div className="aim-activity-icon"><UserCheck className="aim-icon-sm" /></div>
                <div className="aim-activity-content">
                  <p className="aim-activity-text">
                    <span className="aim-activity-user">{activity.intern_name || 'System'}</span> {activity.description}
                  </p>
    ...
    <div key={intern.id} className="aim-intern-card">
      <div className="aim-intern-header">
        <div className="aim-intern-profile">
          <div className="aim-avatar">{intern.full_name.split(' ').map(n => n[0]).join('')}</div>
      ...
      <div className="aim-progress-section">
        <div className="aim-progress-label"><span>Progress</span><span>{intern.progress}%</span></div>
  ...
  const renderInternManagement = () => (
    <div className="aim-section">
      ...
      <div className="aim-card">
        <div className="aim-card-header">
          <div className="aim-filters">
            <div className="aim-search-box">
              ...
              <input
                type="text" placeholder="Search interns..." value={filters.searchTerm}
        ...
        {loading ? (
          <div className="aim-loading"><div className="aim-spinner"></div><p className="aim-loading-text">Loading interns...</p></div>
        ) : (
          <div className="aim-card-body">
            <div className="aim-interns-grid">{data.interns.map(renderInternCard)}</div>
            {filters.totalPages > 1 && (
              <div className="aim-pagination">
                <button onClick={() => setFilters(prev => ({ ...prev, currentPage: Math.max(1, prev.currentPage - 1) }))} 
                        disabled={filters.currentPage === 1} className="aim-btn aim-btn--light">Previous</button>
                <span className="aim-page-info">{filters.currentPage} of {filters.totalPages}</span>
                <button onClick={() => setFilters(prev => ({ ...prev, currentPage: Math.min(filters.totalPages, prev.currentPage + 1) }))} 
                        disabled={filters.currentPage === filters.totalPages} className="aim-btn aim-btn--light">Next</button>
              </div>
            )}
          </div>
        )}
  ...
  const renderTaskManagement = () => (
    <div className="aim-section">
      ...
      <div className="aim-card">
        <div className="aim-card-body">
          {loading ? (
            <div className="aim-loading"><div className="aim-spinner"></div><p className="aim-loading-text">Loading tasks...</p></div>
          ) : (
                ...
                <div key={task.id} className="aim-task-card">
                  ...
                  <div className="aim-task-actions">
                    {buildActionButton(() => {
                      ...
                      setForms(prev => ({ ...prev, task: { title: task.title, description: task.description, assigned_to: task.assigned_to, due_date: task.due_date } }));
      ...
      {buildModal(
        ...
        <>
          {buildFormGroup('Title', 
            <input type="text" value={forms.task.title} onChange={(e) => setForms(prev => ({ ...prev, task: { ...prev.task, title: e.target.value } }))} className="aim-input" />
          )}
          {buildFormGroup('Description', 
            <textarea value={forms.task.description} onChange={(e) => setForms(prev => ({ ...prev, task: { ...prev.task, description: e.target.value } }))} rows={3} className="aim-textarea" />
          )}
          {buildFormGroup('Assign to', 
            <select value={forms.task.assigned_to} onChange={(e) => setForms(prev => ({ ...prev, task: { ...prev.task, assigned_to: e.target.value } }))} className="aim-select">
              <option value="">Select Intern</option>
              {data.interns.filter(intern => intern.status === 'active').map(intern => (
                <option key={intern.id} value={intern.id}>{intern.full_name}</option>
              ))}
            </select>
          )}
          {buildFormGroup('Due Date', 
            <input type="date" value={forms.task.due_date} onChange={(e) => setForms(prev => ({ ...prev, task: { ...prev.task, due_date: e.target.value } }))} className="aim-input" />
          )}
        </>,
  ...
  const renderTabContent = () => {
    const tabs = { dashboard: renderDashboard, interns: renderInternManagement, tasks: renderTaskManagement };
        ...
        <div className="aim-sidebar">
          ...
          <nav className="aim-sidebar-nav">
            {[
              ...
              <button key={tab} className={`aim-nav-item ${activeTab === tab ? 'aim-nav-item--active' : ''}`} onClick={() => setActiveTab(tab)}>
                <Icon className="aim-nav-icon" />{label}
              </button>
            ))}
            <div className="aim-sidebar-footer">
              <button className="aim-sidebar-action"><Settings className="aim-sidebar-action-icon" />Settings</button>
              <button className="aim-sidebar-action"><LogOut className="aim-sidebar-action-icon" />Logout</button>
            </div>
          </nav>
        </div>
