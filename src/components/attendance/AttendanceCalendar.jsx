...
const AttendanceCalendar = () => {
  ...
  const validateCurrentLocation = async (coords) => {
    try {
      const response = await fetch('https://hrms-backend-5wau.onrender.comvalidate-location', {
        method: 'POST',
  ...
  const fetchWorkLocations = async () => {
    try {
      const response = await fetch('https://hrms-backend-5wau.onrender.comwork-locations');
      const result = await response.json();
      if (result.success) {
